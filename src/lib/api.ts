...
function mapSearchItemToVehicle(item: SearchItem, vehicleType: 'car' | 'bike'): Vehicle {
  return {
    ...
    price_usd: item.price ? Math.round((item.price * 10000) / KRW_TO_USD) : 0,
    price_krw: item.price ? item.price * 10000 : 0,
...
export const getCarDetails = async (id: string): Promise<Vehicle | null> => {
  try {
    ...
    return {
      ...
      price_usd: detail.price ? Math.round((detail.price * 10000) / KRW_TO_USD) : 0,
      price_krw: detail.price ? detail.price * 10000 : 0,
